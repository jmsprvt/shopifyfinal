...
const AllProducts: FC = () => {
  ...
  }, [currentPage]); // Обновляем лоадер при изменении страницы

  const indexOfLastProduct = (currentPage + 1) * productsPerPage;
  const indexOfFirstProduct = currentPage * productsPerPage;
  const currentProductsSlice = currentProducts.slice(
    indexOfFirstProduct,
    indexOfLastProduct
  );
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-1">
          ...
          <div
            className={`col-span-4 ${
              windowWidth >= 768 ? "" : "md:col-span-5"
            }`}
          >
            <div className="flex items-center justify-between">
              <div
                className={`flex items-center space-x-2 text-lg ${
                  windowWidth >= 768 ? "" : "hidden md:block"
                }`}
              >
                <span>Products</span>
                <span> {">"} </span>
              ...
              <div className="flex items-center justify-center md:justify-end">
                <input
                  type="text"
                  placeholder="Search..."
                  className={`border border-gray-300 rounded px-2 py-1 focus:border-blue-500 ${
                    window.innerWidth < 365 ? "text-xs" : ""
                  }`}
                  value={searchTerm}
            ...
            <ReactPaginate
              ...
              forcePage={currentPage} // Фиксируем текущую страницу
            />
          </div>
        </div>
      )}
